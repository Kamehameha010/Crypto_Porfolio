(() => {
  ...
  function StoreCredential(credential, wallet2) {
    if (!isExists(credential, wallet2)) {
      let storage = localStorage.getItem(wallet2);
      if (storage) {
        let collection = JSON.parse(storage?.valueOf());
        collection.push(credential);
        localStorage.setItem(wallet2, JSON.stringify(collection));
      }
      return;
    }
  ...
  function isExists(credential, storageName) {
    ...
    if (storage) {
      ...
      console.log(collection.includes(credential));
      return collection.includes(credential);
    }
    return false;
  }
